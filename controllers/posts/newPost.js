  ...
  try {
    ...
    if (isMentionAll) {
      ...
      for (let i = 0; i < validUsersId.length; i++) {
        ...
        if (user.email) await sendMail('post_mention', user.email, { name: req.user.name.split(' ')[0], username: req.user.username, url: `https://twok17.onrender.com/post/${newPost._id}` })
      }
    }
